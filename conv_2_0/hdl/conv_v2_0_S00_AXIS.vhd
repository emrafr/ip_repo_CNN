...
begin
	...
	process (mst_exec_state, S_AXIS_TVALID, writes_done)
	begin
    case (mst_exec_state) is
        when IDLE     => 
          ...
          if (S_AXIS_TVALID = '1') then
            mst_exec_state_next <= WRITE_FIFO;
          else
	  ...
	  if (rising_edge (S_AXIS_ACLK)) then
	    if(S_AXIS_ARESETN = '0') then
	      ...
	      if (write_pointer <= NUMBER_OF_INPUT_WORDS-1) then
	       ...
	       -- fifo cue loads 2 rows becuse ready goes high agian becuse write pointer och state fifo 
